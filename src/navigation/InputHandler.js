...
// three.js's EventDispatcher keeps its listener map as an unexposed internal
// field; potree reaches into it (as `_listeners`) to check whether a scene
// object has any listeners registered for a given custom event type.
/** @param {THREE.Object3D} object @param {string} type */
function hasListeners(object, type){
	const listeners = /** @type {any} */ (object)._listeners;
...
export class InputHandler extends EventDispatcher {
	...
	onDoubleClick(e){
		...
		for(let hovered of this.hoveredElements){
			if(hasListeners(hovered.object, 'dblclick')){
	...
	onMouseDown(e){
		...
		if(!this.drag){
			let target = this.hoveredElements
				.find(el => hasListeners(el.object, 'drag'));
			...
			if(e.button === THREE.MOUSE.LEFT){
				if(noMovement){
					let selectable = this.hoveredElements
						.find(el => hasListeners(el.object, 'select'));
		...
		if(this.drag){
			...
			if(hoveredElements.length > 0){
				let object = hoveredElements
					.map(e => e.object)
					.find(e => hasListeners(e, 'mousemove'));
	...
	onMouseWheel(e){
		...
		let hoveredElement = this.getHoveredElement();
		if(hoveredElement){
			hoveredElement.object.dispatchEvent({
				type: 'mousewheel',
				delta: ndelta,
				object: hoveredElement.object
			});
		}else{
			for(let inputListener of this.getSortedListeners()){
				inputListener.dispatchEvent({
	...
	getHoveredElements(){
		...
		for(let scene of scenes){
			scene.traverseVisible(node => {
				if(/** @type {any} */ (node)._listeners && node.visible && !this.blacklist.has(node)){
					let hasInteractableListener = interactableListeners
						.some(e => hasListeners(node, e));
