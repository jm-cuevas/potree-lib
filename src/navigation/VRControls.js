...
import {debugLine} from "../utils/debug.js";
...
class FlyMode{
	...
	update(vrControls, delta){
		...
		const debugMessage = vrControls.viewer.debug?.message;
		if(debugMessage){
			this.dbgLabel.visible = true;
			this.dbgLabel.setText(debugMessage);
...
class RotScaleMode{
	...
	update(vrControls, delta){
		...
		let mToOrigin = new THREE.Matrix4().makeTranslation(toScene(start_center, this.startState).multiplyScalar(-1));
		let mToStart = new THREE.Matrix4().makeTranslation(toScene(start_center, this.startState));
		...
		let mToNew = new THREE.Matrix4().makeTranslation(toNew);
...
export class VRControls extends EventDispatcher{
	...
	createInfo(){
		const texture = new THREE.TextureLoader().load(`${this.viewer.resourcePath}/images/vr_controller_help.jpg`);
		const plane = new THREE.PlaneGeometry(1, 1, 1, 1);
		const infoMaterial = new THREE.MeshBasicMaterial({map: texture});

		return new THREE.Mesh(plane, infoMaterial);
	}
