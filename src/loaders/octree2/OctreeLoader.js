...
import {createChildAABB} from "../../utils/geometry.js";
import {workerPool, loaderState} from "../LoaderState.js";

const plainWorkerURL = new URL("../../workers/octree2/DecoderWorker.js", import.meta.url).href;
const brotliWorkerURL = new URL("../../workers/octree2/DecoderWorker_brotli.js", import.meta.url).href;
...
export class NodeLoader {
	...
	async load(node) {
		...
		try {
			...
			let workerURL = this.metadata.encoding === "BROTLI" ? brotliWorkerURL : plainWorkerURL;
	...
	parseHierarchy(node, buffer) {
		...
		for (let i = 0; i < numNodes; i++) {
			...
			if (current.byteSize === 0n) {
				// workaround for issue #1125: some inner nodes erroneously
				// report >0 points even though have 0 points; however, they
				// still report a byteSize of 0, so based on that we now set
				// node.numPoints to 0
				current.numPoints = 0;
			}
...
export class OctreeLoader {
	...
	static async load(url) {
		...
		loader.attributes = OctreeLoader.parseAttributes(metadata.attributes);
