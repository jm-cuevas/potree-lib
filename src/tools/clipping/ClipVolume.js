...
/**
 * Legacy single-plane clip box gizmo (a thin oriented box plus a local
 * coordinate cross). Superseded in normal use by `BoxVolume` with
 * `clip = true` and by `PolygonClipVolume`, but ported for completeness.
 *
 * DOM-coupled bits removed vs. upstream: the `select`/`deselect` listeners
 * that toggled a jQuery sidebar `.scene_header` are dropped (the `ui_select`
 * / `ui_deselect` axis-arrow toggles are kept). `offset()` / `rotate()` no
 * longer stuff a bare global `viewer` into their `clip_volume_changed`
 * events, and the `newthis` typo in `rotate()`'s global-space branch is
 * fixed. The upstream `THREE.Geometry` line lists are rebuilt as
 * `BufferGeometry`.
 */
let clipVolumeCounter = 0;
...
export class ClipVolume extends THREE.Object3D{
	...
	constructor(args = {}){
		...
		this.name = "clip_volume_" + (clipVolumeCounter++);
		...
		let boxFrameGeometry = new THREE.BufferGeometry().setFromPoints(boxFramePoints);

		let planeFramePoints = [
			new THREE.Vector3(-0.5, -0.5, 0.0), new THREE.Vector3(-0.5, 0.5, 0.0),
			new THREE.Vector3(0.5, 0.5, 0.0), new THREE.Vector3(0.5, -0.5, 0.0),
			new THREE.Vector3(-0.5, 0.5, 0.0), new THREE.Vector3(0.5, 0.5, 0.0),
			new THREE.Vector3(-0.5, -0.5, 0.0), new THREE.Vector3(0.5, -0.5, 0.0),
		];
		let planeFrameGeometry = new THREE.BufferGeometry().setFromPoints(planeFramePoints);
		...
		let createArrow = (name, direction, color) => {
			...
			let shaftGeometry = new THREE.BufferGeometry().setFromPoints([
	...
	rotate(args) {
		...
		if(cs === "local") {
			...
			let rotaxis3 = new THREE.Vector3(rotaxis.x, rotaxis.y, rotaxis.z);
			this.rotateOnAxis(rotaxis3, dir * this.clipRotOffset * Math.PI / 180);
		}
