...
import {computeCircleCenter, getMousePointCloudIntersection} from "../../utils/geometry.js";

/**
 * Headless measurement object: an ordered list of 3D marker points plus the
 * derived geometry (edges, area, angles, height, fitted circle, azimuth) and
 * the `TextSprite` labels for each. Pure three.js `Object3D` - it renders
 * itself into whatever scene it is added to; the DOM-side measurement panel
 * is left to the consuming app, driven off the `marker_added` /
 * `marker_moved` / `marker_removed` / `marker_dropped` events.
 */
	...
	update () {
		if (this.points.length === 0) {
			...
			{ // coordinate labels
				...
				let msg = position.toArray().map(p => addCommas(p.toFixed(2))).join(" / ");
		...
		for (let i = 0; i <= lastIndex; i++) {
			...
			{ // edge labels
				...
				if(this.lengthUnit != null && this.lengthUnitDisplay != null){
					distance = distance / this.lengthUnit.unitspermeter * this.lengthUnitDisplay.unitspermeter;
					suffix = this.lengthUnitDisplay.code;
				}

				let txtLength = addCommas(distance.toFixed(2));
		...
		{ // update height stuff
			...
			if (this.showHeight) {
				...
				let txtHeight = addCommas(height.toFixed(2));
		...
		{ // update area label
			...
			let suffix = "";
			if(this.lengthUnit != null && this.lengthUnitDisplay != null){
				area = area / Math.pow(this.lengthUnit.unitspermeter, 2) * Math.pow(this.lengthUnitDisplay.unitspermeter, 2);
				suffix = this.lengthUnitDisplay.code;
			}

			let txtArea = addCommas(area.toFixed(1));
