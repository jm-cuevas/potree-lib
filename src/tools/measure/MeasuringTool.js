...
import {projectedRadius} from "../../utils/geometry.js";
import {getNorthVec, computeAzimuth} from "../../utils/geo.js";
import {CameraMode} from "../../core/defines.js";
import {EventDispatcher} from "../../core/EventDispatcher.js";

/**
 * Orchestrates `Measure` insertion and per-frame maintenance: owns a private
 * overlay `THREE.Scene` rendered in the `render.pass.perspective_overlay`
 * pass, drives drag-to-place marker insertion through the viewer's
 * `InputHandler`, and keeps marker/label sizes screen-space constant.
 *
 * The consuming app instantiates it with `new MeasuringTool(viewer)` (core
 * does not depend on it) and calls `startInsertion(...)` from its own UI.
 */
...
function updateAzimuth(viewer, measure){
	...
	const northVec = getNorthVec(p0.position, r, viewer.getProjection());
	...
	{ // north
		...
		let pr = projectedRadius(1, camera, distance, width, height);
	...
	// label
	const radians = computeAzimuth(p0.position, p1.position, viewer.getProjection());
	...
	update(){
		...
		for (let measure of measurements) {
			...
			for(let label of labels){
				...
				if(this.viewer.debug.scale){
					scale = (this.viewer.debug.scale / pr);
				}
