...
import {projectedRadius, mouseToRay} from "../../utils/geometry.js";
import {moveTo} from "../../utils/camera.js";

/**
 * Screen-space-sized transform gizmo (scale / focus / translation / rotation
 * handles) that binds to the current `InputHandler` selection. Pure three.js;
 * `Viewer` reads `viewer.transformationTool?.update()` from its render loop
 * only if a consuming app assigned `viewer.transformationTool = new
 * TransformationTool(viewer)`.
 *
 * `args.resourcePath` (falls back to `viewer.resourcePath`) locates the
 * `icons/eye_2.png` texture for the focus handles; without it the focus
 * handles simply render untextured. The upstream `THREE.Geometry` frame is
 * rebuilt as `BufferGeometry`, and the implicit global `TWEEN` group is the
 * shared `tweens` group.
 */
export class TransformationTool {
	constructor(viewer, args = {}) {
		this.viewer = viewer;
		this.resourcePath = args.resourcePath ?? viewer.resourcePath ?? null;
	...
	initializeScaleHandles(){
		...
		for(let handleName of Object.keys(this.scaleHandles)){
			...
			/** @type {any} */ (pickSphere).handle = handleName;
			this.pickVolumes.push(pickSphere);

			node.setOpacity = (target) => {
				let opacity = {x: material.opacity};
				let t = new TWEEN.Tween(opacity, tweens).to({x: target}, 100);
				t.onUpdate(() => {
	...
	initializeFocusHandles(){
		...
		let texture = this.resourcePath
			? new THREE.TextureLoader().load(`${this.resourcePath}/icons/eye_2.png`)
			: null;
	...
	initializeTranslationHandles(){
		...
		for(let handleName of Object.keys(this.translationHandles)){
			...
			/** @type {any} */ (pickVolume).handle = handleName;
			box.add(pickVolume);
...

			{
...

				{
					...
					if(intersects.length > 0){
						let I = intersects[0];
						let handleName = /** @type {any} */ (I.object).handle;
