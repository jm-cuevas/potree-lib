...
/**
 * Headless profile (cross-section) object: an ordered list of 3D points
 * defining a poly-line, plus per-segment marker spheres, connecting edges
 * and a translucent extraction box (width `this.width`, height `this.height`).
 * Pure three.js `Object3D`; point extraction along the profile is done by
 * `PointCloudOctree.getPointsInProfile()` via `ProfileRequest`. The d3 chart
 * that upstream drew from the extracted points is a UI concern left to the
 * consuming app - drive it off `marker_added` / `marker_moved` /
 * `marker_removed` / `width_changed` events.
 *
 * The upstream `THREE.Geometry` edge lines are rebuilt as `BufferGeometry`
 * (two-vertex position attribute per edge).
 */
...
export class Profile extends THREE.Object3D{
	...
	constructor () {
		...
		this.name = 'Profile_' + (profileCounter++);
		this.points = [];
		this.spheres = [];
		this.edges = [];
		this.boxes = [];
	...
	update () {
		...
		for (let i = 0; i <= lastIndex; i++) {
			...
			if (leftEdge) {
				let pos = leftEdge.geometry.attributes.position;
			...
			if (rightEdge) {
				let pos = rightEdge.geometry.attributes.position;
