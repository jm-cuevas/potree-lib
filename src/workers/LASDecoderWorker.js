// Formats already-decoded LAS/LAZ point records (produced on the main
// thread by LasLazLoader.js via the vendored laz-perf/laslaz.js decoder -
// see src/loaders/las/laslaz.js) into the typed geometry buffers
// LasLazBatcher expects back. This worker does no LAS/LAZ decompression
// itself; it only reinterprets the fixed point-format-2 record layout.
//
// The reference Potree source (src/workers/LASDecoderWorker.js) defined two
// alternative implementations - `readUsingTempArrays` and
// `readUsingDataView` - with only the latter wired up
// (`onmessage = readUsingDataView`). `readUsingTempArrays` was dead code
// (never referenced), and moreover broken (it read `pointSize`, a variable
// that doesn't exist there - only `sourcePointSize` was defined - and it
// had two stray `debugger;` statements), so it isn't carried forward here.
...
onmessage = function (event) {
	...
	for (let i = 0; i < numPoints; i++) {
		...
		let returnNumberAndNumberOfReturns = sourceView.getUint8(i * sourcePointSize + 14);
		...
		// CLASSIFICATION
		let classification = sourceView.getUint8(i * sourcePointSize + 15);
