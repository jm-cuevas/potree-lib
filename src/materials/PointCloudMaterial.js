...
import {generateDataTexture} from "../utils/texture.js";
import {Gradients} from "./Gradients.js";
import {ClassificationScheme} from "./ClassificationScheme.js";
import {PointSizeType, PointShape, TreeType, ElevationGradientRepeat} from "../core/defines.js";
import pointcloudVertexShader from "./shaders/pointcloud.vs?raw";
import pointcloudFragmentShader from "./shaders/pointcloud.fs?raw";
...
export class PointCloudMaterial extends THREE.RawShaderMaterial {
	 ...
	 * @param {number} [parameters.minSize]
	 * @param {number} [parameters.maxSize]
	 * @param {number} [parameters.treeType]
	 * @param {string} [parameters.resourcePath] base path used to load the
	 *   matcap texture set (`${resourcePath}/textures/matcap/${matcap}`).
	 *   Without it, `matcap` shading stays untextured (warns once).
	 */
	constructor(parameters = {}) {
		super();

		this.resourcePath = parameters.resourcePath || null;
		...
		this.matcapTexture = PointCloudMaterial.generateMatcapTexture(this._matcap, this.resourcePath);
		this.lights = false;
		this.fog = false;
		...
		// shader uniforms hold a mix of numbers/arrays/typed arrays/Color/Texture,
		// and several start as an empty `[]` placeholder later replaced with a
		// Float32Array (setClipBoxes()) - annotate broadly rather than fighting
		// checkJs's narrow inference of empty-array-literal initial values.
		/** @type {Object.<string, {value: any}>} */
		this.uniforms = {
			level:              {value: 0.0},
			vnStart:            {value: 0.0},
			spacing:            {value: 1.0},
			blendHardness:      {value: 2.0},
			blendDepthSupplement: {value: 0.0},
			fov:                {value: 1.0},
			screenWidth:        {value: 1.0},
			screenHeight:       {value: 1.0},
			near:               {value: 0.1},
			far:                {value: 1.0},
			...
			octreeSize:         {value: 0},
			bbSize:             {value: [0, 0, 0]},
			elevationRange:     {value: [0, 0]},

			clipBoxCount:       {value: 0},
			clipPolygonCount:   {value: 0},
			clipBoxes:          {value: []},
			clipPolygons:       {value: []},
			clipPolygonVCount:  {value: []},
			clipPolygonVP:      {value: []},

			visibleNodes:       {value: this.visibleNodesTexture},
			pcIndex:            {value: 0},
			gradient:           {value: this.gradientTexture},
			classificationLUT:  {value: this.classificationTexture},
			uHQDepthMap:        {value: null},
			toModel:            {value: []},
			diffuse:            {value: [1, 1, 1]},
			transition:         {value: 0.5},

			intensityRange:     {value: [Infinity, -Infinity]},

			intensity_gbc:      {value: [1, 0, 0]},
			uRGB_gbc:           {value: [1, 0, 0]},
			wRGB:               {value: 1},
			wIntensity:         {value: 0},
			wElevation:         {value: 0},
			wClassification:    {value: 0},
			wReturnNumber:      {value: 0},
			wSourceID:          {value: 0},
			useOrthographicCamera: {value: false},
			elevationGradientRepat: {value: ElevationGradientRepeat.CLAMP},
			clipTask:           {value: 1},
			clipMethod:         {value: 1},
			uShadowColor:       {value: [0, 0, 0]},

			uExtraScale:        {value: 1},
			uExtraOffset:       {value: 0},
			uExtraRange:        {value: [0, 1]},
			uExtraGammaBrightContr: {value: [1, 0, 0]},
		...
		this.vertexShader = pointcloudVertexShader;
		this.fragmentShader = pointcloudFragmentShader;
	...
	/**
	 * Widens THREE.Material's `dispose`-only event map to the ad-hoc custom
	 * event types (`material_property_changed`, `opacity_changed`, ...) this
	 * class actually dispatches - see src/types/three-events.d.ts for why
	 * this can't be done via module augmentation like Object3DEventMap.
	 *
	 * @param {{type: string, [key: string]: any}} event
	 */
	dispatchEvent(event) {
		super.dispatchEvent(/** @type {any} */ (event));
	}
	...
	updateShaderSource() {
		let vs = pointcloudVertexShader;
		let fs = pointcloudFragmentShader;
	...
	recomputeClassification() {
		...
		for (let i = 0; i < width; i++) {
			...
			const r = Math.trunc(255 * color[0]);
			const g = Math.trunc(255 * color[1]);
			const b = Math.trunc(255 * color[2]);
			const a = visible ? Math.trunc(255 * color[3]) : 0;
	...
	// @ts-expect-error TS2611: three.js declares Material.opacity as a plain
	...
	/**
	 * @param {string} matcap texture file name, relative to
	 *   `${resourcePath}/textures/matcap/`.
	 * @param {?string} resourcePath
	 */
	static generateMatcapTexture(matcap, resourcePath) {
		if (!resourcePath) {
			console.warn("PointCloudMaterial: matcap shading requires the `resourcePath` constructor option; matcap texture not loaded.");
			return null;
		}

		let url = new URL(`${resourcePath}/textures/matcap/${matcap}`, location.href).href;
