/**
 * Serialises a {@link Points}-shaped result set to an uncompressed LAS 1.2
 * point-data-record-format-2 buffer (XYZ + intensity + return info +
 * classification + RGB). Returns the raw `ArrayBuffer`; writing it to a file is
 * left to the caller. Pure data transform - three.js is used only for the
 * `Vector3` scale/offset math.
 */
...
export class LASExporter {
	 ...
	 *     returnNumber?: ArrayLike<number>,
	 *     numberOfReturns?: ArrayLike<number>,
	 *     classification?: ArrayLike<number>,
	 *     pointSourceID?: ArrayLike<number>,
	 *     rgba?: ArrayLike<number>,
	 *     color?: ArrayLike<number>,
	 *   },
	 * }} points
	 * @returns {ArrayBuffer} a complete LAS 1.2 file
	 */
	static toLAS (points) {
		...
		setString('potree-lib', 58, buffer);
		...
		for (let i = 0; i < points.numPoints; i++) {
			...
			let ux = Math.trunc((px - offset.x) / scale.x);
			let uy = Math.trunc((py - offset.y) / scale.y);
			let uz = Math.trunc((pz - offset.z) / scale.z);
