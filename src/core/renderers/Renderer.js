...
class Shader {
	...
	linkProgram(){
		...
		this.cache.set(`${this.vsSource}, ${this.fsSource}`, toCache);
	}
...
// subset of THREE.WebGLRenderer's internal param mapping, kept local since this
// renderer talks to WebGL directly instead of going through THREE.WebGLRenderer
...
/**
 * Raw-WebGL renderer for potree's point cloud octrees. Bypasses
 * THREE.WebGLRenderer for the point cloud draw calls (manual shader/buffer/
 * texture caching) since standard three.js materials/geometries can't express
 * the per-node LOD/visibility-texture pipeline potree relies on.
 *
 * Only understands `PointCloudTree` scene graph nodes - everything else is
 * skipped when traversing the scene passed to `render()`.
 */
export class Renderer {
	...
	constructor(threeRenderer){
		...
		this.debug = {allowedNodes: undefined};
	}
	...
	renderNodes(octree, nodes, visibilityTextureData, camera, target, shader, params){
		if(this.measureTimings) performance.mark("renderNodes-start");
		...
		for(let node of nodes){
			if(this.debug.allowedNodes !== undefined){
				if(!this.debug.allowedNodes.includes(node.name)){
					continue;
				}
			}
	...
	renderOctree(octree, nodes, camera, target, params = {}){
		...
		{ // UPDATE SHADER AND TEXTURES
			if(!this.shaders.has(material)){
				...
				let newShader = new Shader(gl, "pointcloud", vs, fs);

				this.shaders.set(material, newShader);
			}
			...
			for(let uniformName of Object.keys(material.uniforms)){
				...
				if(uniform.value instanceof THREE.Texture){
					...
					if(!this.textures.has(texture)){
						let webglTexture = new GLTexture(gl, texture);
