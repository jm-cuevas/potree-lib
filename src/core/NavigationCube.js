...
/**
 * Clickable F/B/L/R/U/D cube, rendered by the consuming app in its own
 * viewport corner (see `Viewer.navigationCube`, opt-in via `NavigationCube.width`
 * and manual `renderer.render(navigationCube, navigationCube.camera)`).
 * Face textures are loaded from `${viewer.resourcePath}/textures/navigation/`.
 */
		...
		let createPlaneMaterial = (img) => {
			...
			if(viewer.resourcePath){
				new THREE.TextureLoader().load(
					`${viewer.resourcePath}/textures/navigation/${img}`,
					function(texture){
